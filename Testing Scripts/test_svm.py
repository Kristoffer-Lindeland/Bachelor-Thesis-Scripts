...
MODEL_PATH = r"C:\path\to\model\svm_{}.joblib"
NUM_COEFFS = 15
MAP_SIZE = (32, 32)
OUTPUT_CSV = r"C:\path\to\SVM_test_predictions.csv"
...
def extract_dct_map_flat(image_path, num_coeffs=NUM_COEFFS, out_shape=MAP_SIZE):
...
def main():
    ...
    for alg in algorithms:
        print(f"Loading model for {alg}...")
        scaler, model = joblib.load(MODEL_PATH.format(alg))
        models[alg] = (scaler, model)
    ...
    for fname in tqdm(test_filenames, desc="Processing test images"):
        ...
        feat = extract_dct_map_flat(path)
        ...
        for model_alg, (scaler, model) in models.items():
